test_that("getName works", {
 expect_equal(
   purrr::map_chr(
     c("xyz.txt","./xyz.txt", "xyz", "~/xyz.txt", "../xyz.txt", "c:/abc/def/xyz.txt", "../abc/xyz.txt.gz"),
  \(x) sdamisc::getName(x)), rep("xyz", 7))
  expect_equal(getName("c:\\xyz.txt"), "c:\\xyz")
})

df1 <- tibble("chr" = rep("chr1", 4),
                  "start" = c(100, 300, 600, 800),
                  "end" = c(200, 400, 700, 900))
bp1 <- tibble("chr" = rep("chr1", 8),
...
bp2 <- tibble(bp1, "meta" = c("record1", "record1", "record2", "record2", "record3", "record3", "record4", "record4"))
